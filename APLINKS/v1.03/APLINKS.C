/***************************************************************
    ...
    ---------------------------------------------------------------
    Modernized for ISO C (C99).  The virtual-disk logic and the
    ...
    Build:
      Windows : gcc -x c -std=c99 -O2 -o aplinks32.exe APLINKS.C
                cl  /O2 APLINKS.C
...
int	g_loaded       = 0;        /* files preloaded so far              */
int	g_verbose      = 0;        /* -v : log every command + sector     */
int	g_rts          = 1;        /* RTS line: 1=ON. The pocket only      */
...
int main(int argc, char *argv[])
{
	...
	switch ( cmd ) {
		...
		case 'D':
		    ...
		    epilogue( 1 );          /* flush, then keep serving */
		    break;
		default:
